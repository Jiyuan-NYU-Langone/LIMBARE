...
#' Details
#'
#' @param object a fitted segmented model coming from segmented.lm or segmented.glm
...
predict.segmented1 <-function(object, newdata, .coef=NULL, ...){
  ...
  # f<-if(inherits(object, what = "glm", which = FALSE)) predict.glm(object, newdata=newd.ok, ...)
  # else predict.lm(object, newdata=newd.ok, ...)
  return(f)
  #sommare se "terms"?
}
